...
// Set an interval to make stars disappear at sunrise
setInterval(() => {
    makeStarsDisappear();
}, 10 * 60 * 1000); // Check every 10 minutes

// Function to make stars disappear at sunrise
function makeStarsDisappear() {
    const hours = new Date().getHours();

    if (hours >= 5 && hours < 6) { // Adjust the timeframe for sunrise
        const stars = document.querySelectorAll('.star');
        stars.forEach((star) => {
            star.style.opacity = '0';
        });
